...
<div class="admin-shell stack">
    ...
    <section class="table-shell">
        ...
        <div class="panel-body"><table class="page-table"><thead><tr><th>Página</th><th>Secciones</th><th>Estado</th><th>Último cambio</th><th>Acciones</th></tr></thead><tbody>
            @forelse($pages as $page)
                <tr data-search-row><td><strong>{{ $page->name }}</strong><br><span class="muted">/{{ $page->slug }}</span>@if($hasEmsEditor && $page->id === $homePage->id)<br><a class="back-link" style="margin:4px 0 0" href="{{ route('admin.pages.edit', ['page' => $page, 'tab' => 'ems']) }}">Incluye el contenido de EMS ↗</a>@endif</td><td>{{ $page->sections_count }}</td><td><span class="pill {{ $page->is_active ? 'pill-ok' : 'pill-off' }}">{{ $page->is_active ? 'Publicada' : 'Inactiva' }}</span></td><td><span class="muted">{{ $page->updated_at?->format('d/m/Y H:i') ?? '—' }}</span></td><td><a class="button button-secondary" href="{{ route('admin.pages.edit', $page) }}">Editar página</a></td></tr>
    ...
    <p class="section-copy">El logo, los colores, el encabezado y el pie se administran desde <a class="back-link" href="{{ route('admin.global.edit') }}">Configuración global</a>. EMS se edita como una sección de Inicio (Home).</p>
</div>
@endsection
